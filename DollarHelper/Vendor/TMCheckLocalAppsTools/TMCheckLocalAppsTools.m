...
//    NSLog(@"%@", apps);
    ...
    //  iOS7->"LSApplicationProxy: com.yunva.yaya.fdadsgfdas"
    //  iOS8->"<LSApplicationProxy: 0x135e12bc0> com.apple.PassbookUIService"
    
    // 判断系统版本
    NSString *separateString = nil;
    if ([UIDevice currentDevice].systemVersion.floatValue < 8.0) {// iOS8以下
        separateString = @": ";
    }else {
        separateString = @"> ";
    }
    ...
    for (NSString *tempStr in tempArray) {
        ...
        NSArray *subStrArray1 = [tempStr componentsSeparatedByString:separateString];
